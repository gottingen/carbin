...
import sys, os, tarfile, carbin.util, shutil
...
__carbin_exe__ = carbin.util.which('carbin')

__has_pkg_config__ = carbin.util.can(lambda: carbin.util.which('pkg-config'))
...
class DirForTests:
    ...
    def cmd(self, *args, **kwargs):
        ...
        carbin.util.cmd(*args, shell=True, **kwargs)
    ...
    def write_to(self, f, content):
        p = self.get_path(f)
        carbin.util.write_to(p, content)
        return p
...
def install_cmds(url, lib=None, alias=None, init=None, remove='remove', list_='list', size=1, recipes=None, prefix=None, build_path=None, variants=None, base_size=0, files=None):
    cg = CarbinCmd(prefix=prefix, build_path=build_path)
...
if carbin.util.USE_SYMLINKS:
    def test_local_dir(d):
        carbin.util.copy_to(get_exists_path('libsimple'), d.tmp_dir)
...
def test_recipe_simple(d):
    recipes=get_exists_path('basicrecipes') + ' -DCARBIN_TEST_DIR="' + __test_dir__ + '"'
...
def test_symlink_in_prefix(d):
    p = d.get_path('usr')
    if carbin.util.USE_SYMLINKS:
        carbin.util.mkdir(os.path.join(p, 'share', 'man'))
...
def test_single_header_xcmake(d):
    ...
    url = f + ' --cmake header'
    d.cmds(install_cmds(url=url, alias=f, files=['carbin/include/simple.h']))
...
def test_header_xcmake(d):
    ...
    d.cmds(install_cmds(url=url, alias=get_exists_path('simpleinclude'), files=['carbin/include/simple.h']))

def test_header_xcmake_include(d):
    url = get_exists_path('simpleheader') + ' --cmake header -DINCLUDE_DIR=inc'
    d.cmds(install_cmds(url=url, alias=get_exists_path('simpleheader'), files=['carbin/include/simple/simple.h']))
...
def test_update(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('simpleapp')),
        carbin_cmd('size', '1'),
        carbin_cmd('install', '--verbose --test --update', 'app,'+get_exists_path('simpleapp')),
        carbin_cmd('size', '1'),
        carbin_cmd('rm', '--verbose -y', 'app'),
        carbin_cmd('size', '0')
    ])
...
def test_update_reqs(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('simpleapp')),
        carbin_cmd('size', '1'),
        carbin_cmd('install', '--verbose --test --update', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2'),
        carbin_cmd('rm', '--verbose -y', 'app'),
        carbin_cmd('size', '1')
    ])
...
def test_rm_all(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test --update', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2'),
        carbin_cmd('rm', '--verbose -y --all'),
        carbin_cmd('size', '0')
    ])

def test_unlink1(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', 'simple,'+get_exists_path('libsimple')),
        carbin_cmd('size', '1'),
        carbin_cmd('rm', '--verbose -y --unlink', 'simple'),
        carbin_cmd('size', '0'),
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2'),
        carbin_cmd('rm', '--verbose -y', 'app'),
        carbin_cmd('size', '1'),
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('basicapp')),
...
def test_unlink3(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2'),
        carbin_cmd('rm', '--verbose -y --unlink', 'app'),
        carbin_cmd('size', '1'),
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2'),
        carbin_cmd('rm', '--verbose -y', 'app'),
...
def test_unlink4(d):
    d.cmds([
        ...
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2'),
        carbin_cmd('rm', '--verbose -y', 'simple'),
        carbin_cmd('size', '0'),
        carbin_cmd('install', '--verbose --test', 'app,'+get_exists_path('basicapp')),
        carbin_cmd('size', '2')
    ])
...
def test_unlink_shared(d):
    d.cmds([
        carbin_cmd('init', '--shared'),
        carbin_cmd('install', '--verbose --test', get_exists_path('libsimple2')),
        carbin_cmd('size', '1')
    ])
    libname = None
    ...
    if libname:
        d.assert_path('carbin', 'lib', libname)
    d.cmds([
        carbin_cmd('rm', '--verbose -y --unlink', get_exists_path('libsimple2')),
        carbin_cmd('install', '--verbose', get_exists_path('libsimple2')),
        carbin_cmd('size', '1')
    ])
    if libname:
        d.assert_path('carbin', 'lib', libname)
...
def test_unlink_pkg(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', get_exists_path('libsimple')),
        carbin_cmd('size', '1'),
        carbin_cmd('rm', '--verbose -y --unlink', get_exists_path('libsimple')),
    ])
    shutil.rmtree(d.get_path('carbin', 'carbin', 'pkg'))
    d.cmds([
        carbin_cmd('install', '--verbose --test', get_exists_path('libsimple')),
        carbin_cmd('size', '1')
    ])
...
def test_build_dir_notest(d):
    d.cmds([
        carbin_cmd('build', '--verbose', get_exists_path('buildtesting')),
    ])
...
def test_build_dir_with_test(d):
    d.cmds([
        carbin_cmd('build', '--verbose --test', get_exists_path('buildtesting')),
    ])
...
def test_build_current_dir(d):
    ...
    d.cmds(build_cmds(prefix=d.get_path('carbin')), cwd=cwd)

def test_build_target(d):
    d.cmds([
        carbin_cmd('build', '--verbose', '--target simpleapp', get_exists_path('simpleapp')),
        carbin_cmd('build', '--verbose', '--target simpleapptest', get_exists_path('simpleapp')),
        carbin_cmd('size', '0')
    ])
...
def test_build_install_target(d):
    # d.mkdir('app')
    d.mkdir('tmp').cmds([
        carbin_cmd('build', '--verbose', '--target install', '-DCMAKE_INSTALL_PREFIX="' + d.get_path('app') + '"', get_exists_path('basicapp')),
        carbin_cmd('size', '1')
    ])
...
def test_build_target_fail(d):
    d.cmds([carbin_cmd('build', '--verbose', '--target xyz', get_exists_path('simpleapp'))])
...
def test_init_cpp11(d):
    d.cmds([
        carbin_cmd('init', '--std=c++0x'),
        carbin_cmd('install', '--verbose --test', get_exists_path('libsimple11'))
    ])
...
def test_reqs_file_flag(d):
    reqs_file = d.write_to('reqs', ['-f'+shlex_quote(get_exists_path('basicapp', 'carbin_deps.txt'))])
    d.cmds(install_cmds(url='--file {}'.format(reqs_file), lib='simple', alias='simple'))
...
def test_reqs_f2(d):
    reqs_file = get_exists_path('basicapp', 'carbin_deps.txt')
    d.cmds(install_cmds(url='-f {}'.format(reqs_file), lib='simple', alias='simple'))

@appveyor_skip
def test_reqs_f3(d):
    reqs_file = d.write_to('reqs', [shlex_quote(get_exists_path('libsimple'))])
    d.write_to('carbin_deps.txt', [shlex_quote(get_exists_path('basicapp'))])
    d.cmds(install_cmds(url='-f {}'.format(reqs_file), lib='simple', alias=get_exists_path('libsimple')))

def test_reqs_hash(d):
    ar = d.get_path('libsimple.tar.gz')
    create_ar(archive=ar, src=get_exists_path('libsimple'))
    h = carbin.util.hash_file(ar, 'sha1')
...
def test_without_reqs_f(d):
    d.write_to('carbin_deps.txt', [shlex_quote(get_exists_path('libsimple'))])
    d.cmds(install_cmds(url='', lib='simple', alias=get_exists_path('libsimple')))

@appveyor_skip
def test_without_reqs_f_dev(d):
    d.write_to('dev-carbin_deps.txt', [shlex_quote(get_exists_path('libsimple'))])
    d.cmds(install_cmds(url='', lib='simple', alias=get_exists_path('libsimple')))

@appveyor_skip
def test_without_reqs_f_dev2(d):
    d.write_to('dev-carbin_deps.txt', [shlex_quote(get_exists_path('libsimple'))])
    d.write_to('carbin_deps.txt', ['fail'])
    d.cmds(install_cmds(url='', lib='simple', alias=get_exists_path('libsimple')))
...
def test_reqs_recipe(d):
    recipes=shlex_quote(get_exists_path('basicrecipes')) + ' -DCARBIN_TEST_DIR="' + __test_dir__ + '"'
...
if __has_pkg_config__:
    ...
    def test_build_flag_child(d):
        d.cmds([
            carbin_cmd('install', '--verbose', get_exists_path('basicappbuild')),
            carbin_cmd('list'),
            carbin_cmd('size', '2'),
            carbin_cmd('remove', '-y simple'),
            carbin_cmd('list'),
            carbin_cmd('size', '1')
        ])
    ...
    def test_build_flag_parent(d):
        d.cmds([
            ...
            carbin_cmd('list'),
            carbin_cmd('size', '2'),
            carbin_cmd('remove', '-y', get_exists_path('basicappbuild')),
            carbin_cmd('list'),
            carbin_cmd('size', '1')
        ])
...
def test_install_simple_app_test_with_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', get_exists_path('simpleapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '2'),
        carbin_cmd('remove', '-y simple'),
        carbin_cmd('list'),
...
def test_install_simple_app_test_with_test_all(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test-all', get_exists_path('simpleapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '2'),
        carbin_cmd('remove', '-y simple'),
        carbin_cmd('list'),
...
def test_install_simple_app_test_all_with_test_with_all(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test-all', get_exists_path('simpleapptestall')),
        carbin_cmd('list'),
        carbin_cmd('size', '3'),
        carbin_cmd('remove', '-y simple'),
        carbin_cmd('list'),
...
def test_install_simple_app_test_all_with_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', get_exists_path('simpleapptestall')),
        carbin_cmd('list'),
        carbin_cmd('size', '2'),
        carbin_cmd('remove', '-y simpleapptest'),
        carbin_cmd('list'),
        carbin_cmd('size', '1')
    ])
...
def test_install_simple_app_test_all_without_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose', get_exists_path('simpleapptestall')),
        carbin_cmd('list'),
        carbin_cmd('size', '1'),
        carbin_cmd('remove', '-y', get_exists_path('simpleapptestall')),
        carbin_cmd('list'),
        carbin_cmd('size', '0')
    ])
...
def test_install_simple_app_test_without_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose', get_exists_path('simpleapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '1')
    ])
...
def test_build_simple_app_test_without_test(d):
    d.cmds([
        carbin_cmd('build', '--verbose', get_exists_path('simpleapptest')),
        carbin_cmd('size', '0')
    ])
...
def test_install_simple_basic_app_test_with_test_all(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test-all', get_exists_path('simplebasicapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '3'),
        carbin_cmd('remove', '-y simple'),
        carbin_cmd('list'),
...
def test_install_simple_basic_app_test_with_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', get_exists_path('simplebasicapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '3'),
        carbin_cmd('remove', '-y simple'),
        carbin_cmd('list'),
...
def test_install_simple_basic_app_test_without_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose ', get_exists_path('simplebasicapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '1'),
        carbin_cmd('remove', '-y', get_exists_path('simplebasicapptest')),
        carbin_cmd('list'),
        carbin_cmd('size', '0')
    ])
...
def test_install_simple_basic_app2_test_with_test_all(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test-all', get_exists_path('simplebasicapp')),
        carbin_cmd('list'),
        carbin_cmd('size', '4'),
        carbin_cmd('remove', '-y simple'),
        carbin_cmd('list'),
...
def test_install_simple_basic_app2_test_with_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose --test', get_exists_path('simplebasicapp')),
        carbin_cmd('list'),
        carbin_cmd('size', '2'),
        carbin_cmd('remove', '-y simplebasicapptest'),
        carbin_cmd('list'),
        carbin_cmd('size', '1')
    ])
...
def test_install_simple_basic_app2_test_without_test(d):
    d.cmds([
        carbin_cmd('install', '--verbose ', get_exists_path('simplebasicapp')),
        carbin_cmd('list'),
        carbin_cmd('size', '1'),
        carbin_cmd('remove', '-y', get_exists_path('simplebasicapp')),
        carbin_cmd('list'),
        carbin_cmd('size', '0')
    ])
...
def test_flags_fail(d):
    d.cmds([carbin_cmd('install', '--verbose --test -DCARBIN_FLAG=Off', get_path('libsimpleflag'))])

def test_flags(d):
    p = get_exists_path('libsimpleflag')
    d.cmds(install_cmds(url='-DCARBIN_FLAG=On {}'.format(p), alias=p))

@appveyor_skip
def test_flags_init(d):
    d.cmds(install_cmds(init='-DCARBIN_FLAG=On', url=get_exists_path('libsimpleflag')))

def test_build_flags(d):
    d.cmds(build_cmds(get_exists_path('libsimpleflag'), defines='-DCARBIN_FLAG=On'))

@appveyor_skip
def test_build_flags_init(d):
    d.cmds(build_cmds(init='-DCARBIN_FLAG=On', url=get_exists_path('libsimpleflag')))

@appveyor_skip
def test_flags_init_integer(d):
    d.cmds(install_cmds(init='-DCARBIN_FLAG=1', url=get_exists_path('libsimpleflag')))

@appveyor_skip
@pytest.mark.xfail(strict=True)
def test_flags_fail_integer(d):
    d.cmds([carbin_cmd('install --verbose --test -DCARBIN_FLAG=0', get_path('libsimpleflag'))])

@appveyor_skip
def test_flags_integer(d):
    p = get_exists_path('libsimpleflag')
    d.cmds(install_cmds(url='-DCARBIN_FLAG=1 {}'.format(p), alias=p))

@appveyor_skip
@pytest.mark.xfail(strict=True)
def test_flags_fail_define(d):
    d.cmds([carbin_cmd('install', '--verbose --test --define CARBIN_FLAG=Off', get_path('libsimpleflag'))])

def test_flags_define(d):
    d.cmds([carbin_cmd('install', '--verbose --test --define CARBIN_FLAG=On', get_exists_path('libsimpleflag'))])

@appveyor_skip
def test_flags_toolchain(d):
    d.cmds([
        carbin_cmd('init', '--toolchain', get_toolchain('toolchainflag.cmake')),
        carbin_cmd('install', '--verbose --test', get_exists_path('libsimpleflag'))
    ])

def test_flags_toolchain_prefix(d):
    cg = CarbinCmd(d.get_path('usr'))
    d.cmds([
...
def test_flags_reqs_f(d):
    p = get_exists_path('libsimpleflag')
    reqs_file = d.write_to('reqs', [shlex_quote(p) + ' -DCARBIN_FLAG=On'])
...
def test_flags_reqs_f2(d):
    ...
    d.cmds(install_cmds(url='-f {} -DCARBIN_FLAG=On'.format(reqs_file), alias=p))
...
def test_flags_reqs_f2_fail(d):
    p = get_exists_path('libsimpleflag')
    reqs_file = d.write_to('reqs', [shlex_quote(p)])
    d.cmds(install_cmds(url='-f {} -DCARBIN_FLAG=Off'.format(reqs_file), alias=p))

def test_multiflags(d):
    p = get_exists_path('libsimplemultiflag')
    d.cmds(install_cmds(url='-DCARBIN_FLAG1=On -DCARBIN_FLAG2=On {}'.format(p), alias=p))

@appveyor_skip
@pytest.mark.xfail(strict=True)
def test_multiflags_fail1(d):
    d.cmds([carbin_cmd('install', '--verbose --test -DCARBIN_FLAG1=Off -DCARBIN_FLAG2=Off', get_path('libsimplemultiflag'))])

@appveyor_skip
@pytest.mark.xfail(strict=True)
def test_multiflags_fail2(d):
    d.cmds([carbin_cmd('install', '--verbose --test -DCARBIN_FLAG1=Off -DCARBIN_FLAG2=On', get_path('libsimplemultiflag'))])

@appveyor_skip
@pytest.mark.xfail(strict=True)
def test_multiflags_fail3(d):
    d.cmds([carbin_cmd('install', '--verbose --test -DCARBIN_FLAG1=On', get_path('libsimplemultiflag'))])
...
def test_ignore(d):
    d.cmds([
        carbin_cmd('size', '0'),
        carbin_cmd('ignore', '--verbose', get_exists_path('basicapp')),
        carbin_cmd('size', '1'),
        carbin_cmd('install', '--verbose', get_exists_path('basicapp')),
        carbin_cmd('size', '1'),
        carbin_cmd('remove', '--verbose -y', get_exists_path('basicapp')),
        carbin_cmd('size', '0')
    ])
...
def test_ignore_dep(d):
    d.cmds([
        carbin_cmd('ignore', '--verbose simple'),
        carbin_cmd('install', '--verbose', get_exists_path('basicapp'))
    ])
...
def test_symlink_dir(d):
    d.cmds([
        carbin_cmd('install', get_path('symlinkdir'))
    ])
    assert os.path.exists(d.get_path('carbin', 'data', 'sdir', 'file.txt'))

def test_cmake_trouble(d):
    d.cmds([
        carbin_cmd('install', get_exists_path('cmake-trouble')),
        carbin_cmd('install', get_exists_path('libsimple'))
    ])
